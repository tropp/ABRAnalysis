...
class conversion():
	...
	def Pullfiles(self,directory=''):
		...
		for i in range(len(ABRfiles)):
			...
			if state is False:
				header2 = ":SW EAR: R	SW FREQ: " + str(self.freqs[i]) + ".00	# AVERAGES: 512	REP RATE (/sec): 40	DRIVER: Starship	SAMPLE (usec): 10 \n"
			else:
				header2 = ":SW EAR: R	SW FREQ: 1	# AVERAGES: 512	REP RATE (/sec): 40	DRIVER: Starship	SAMPLE (usec): 10 \n"
			header3 = ":NOTES- \n"
			header4 = ":CHAMBER-412 \n"
	...
	def combinepolarity(self,mypath,onlyfiles):
		...
		if len(fl1)==0:
			print 'up in here'
			nff = [genfile for genfile in nf if 'n' in genfile]
			print 'negative file:', nff
